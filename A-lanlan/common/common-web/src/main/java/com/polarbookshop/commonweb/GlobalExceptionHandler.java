...
public class GlobalExceptionHandler {
    ...
    public ResultBox<?> handleBusinessException(BusinessException e) {
        // 业务异常通常是用户行为（如密码错误），用 WARN 级别记录即可
        log.warn("业务阻断: code={}, msg={}", e.getCode(), e.getMessage());

        // 转化为标准的 JSON 返回体
        return ResultBox.fail(e.getCode(), e.getMessage());
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResultBox<?> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        return ResultBox.fail("A0105", e.getMessage());
    }

    // 🎯 拦截点 2：可预期的系统异常（如下游服务超时、资源不可用）
    @ExceptionHandler(SystemException.class)
    public ResultBox<?> handleSystemException(SystemException e) {
        log.error("系统异常: code={}, msg={}", e.getCode(), e.getMessage(), e);
        return ResultBox.fail(e.getCode(), "服务器开小差了，请稍后再试");
    }

    // 🎯 拦截点 3：兜底拦截所有未知崩溃 (如 NullPointerException)
    @ExceptionHandler(Exception.class)
    public ResultBox<?> handleUnknownException(Exception e) {
        log.error("系统未知异常: ", e);
        return ResultBox.fail("B0001", "服务器开小差了，请稍后再试");
    }
